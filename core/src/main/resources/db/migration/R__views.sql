...
DROP VIEW IF EXISTS `volume_avg_by_banner_in_county`;
-- Creating temporary table to overcome VIEW dependency errors
...
DROP VIEW IF EXISTS `volume_avg_by_county`;
-- Creating temporary table to overcome VIEW dependency errors
...
DROP VIEW IF EXISTS `volume_avg_by_fit_in_county`;
-- Creating temporary table to overcome VIEW dependency errors
...
CREATE VIEW `most_recent_shopping_center_casing` AS select s1.* from (`shopping_center_casing` `s1` left join `shopping_center_casing` `s2` on(((`s1`.`shopping_center_id` = `s2`.`shopping_center_id`) and ((`s1`.`shopping_center_casing_date` < `s2`.`shopping_center_casing_date`) or ((`s1`.`shopping_center_casing_date` = `s2`.`shopping_center_casing_date`) and (`s1`.`shopping_center_casing_id` < `s2`.`shopping_center_casing_id`)))))) where isnull(`s2`.`shopping_center_id`);

-- Dumping structure for view mtn_rest.most_recent_shopping_center_survey
DROP VIEW IF EXISTS `most_recent_shopping_center_survey`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_shopping_center_survey`;
CREATE VIEW `most_recent_shopping_center_survey` AS select s1.* from (`shopping_center_survey` `s1` left join `shopping_center_survey` `s2` on(((`s1`.`shopping_center_id` = `s2`.`shopping_center_id`) and ((`s1`.`shopping_center_survey_date` < `s2`.`shopping_center_survey_date`) or ((`s1`.`shopping_center_survey_date` = `s2`.`shopping_center_survey_date`) and (`s1`.`shopping_center_survey_id` < `s2`.`shopping_center_survey_id`)))))) where isnull(`s2`.`shopping_center_id`);

-- Dumping structure for view mtn_rest.most_recent_store_casing
DROP VIEW IF EXISTS `most_recent_store_casing`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_casing`;
CREATE VIEW `most_recent_store_casing` AS select s1.* from (`store_casing` `s1` left join `store_casing` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and ((`s1`.`store_casing_date` < `s2`.`store_casing_date`) or ((`s1`.`store_casing_date` = `s2`.`store_casing_date`) and (`s1`.`store_casing_id` < `s2`.`store_casing_id`)))))) where isnull(`s2`.`store_id`);

-- Dumping structure for view mtn_rest.most_recent_store_model
DROP VIEW IF EXISTS `most_recent_store_model`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_model`;
CREATE VIEW `most_recent_store_model` AS select s1.* from (`store_model` `s1` left join `store_model` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and ((`s1`.`model_date` < `s2`.`model_date`) or ((`s1`.`model_date` = `s2`.`model_date`) and (`s1`.`store_model_id` < `s2`.`store_model_id`)))))) where isnull(`s2`.`store_id`);

-- Dumping structure for view mtn_rest.most_recent_store_survey
DROP VIEW IF EXISTS `most_recent_store_survey`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_survey`;
CREATE VIEW `most_recent_store_survey` AS select s1.* from (`store_survey` `s1` left join `store_survey` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and ((`s1`.`store_survey_date` < `s2`.`store_survey_date`) or ((`s1`.`store_survey_date` = `s2`.`store_survey_date`) and (`s1`.`store_survey_id` < `s2`.`store_survey_id`)))))) where isnull(`s2`.`store_id`);

-- Dumping structure for view mtn_rest.most_recent_store_volume
DROP VIEW IF EXISTS `most_recent_store_volume`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_volume`;
CREATE VIEW `most_recent_store_volume` AS select s1.* from (`store_volume` `s1` left join `store_volume` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and ((`s1`.`volume_date` < `s2`.`volume_date`) or ((`s1`.`volume_date` = `s2`.`volume_date`) and (`s1`.`store_volume_id` < `s2`.`store_volume_id`)))))) where isnull(`s2`.`store_id`);

-- Dumping structure for view mtn_rest.most_recent_store_volume_actual
DROP VIEW IF EXISTS `most_recent_store_volume_actual`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_volume_actual`;
CREATE VIEW `most_recent_store_volume_actual` AS select s1.* from (`store_volume_actual` `s1` left join `store_volume_actual` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and (`s1`.`volume_date` < `s2`.`volume_date`)))) where isnull(`s2`.`store_id`) order by `s1`.`store_id`;

-- Dumping structure for view mtn_rest.most_recent_store_volume_estimate
DROP VIEW IF EXISTS `most_recent_store_volume_estimate`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_volume_estimate`;
CREATE VIEW `most_recent_store_volume_estimate` AS select s1.* from (`store_volume_estimate` `s1` left join `store_volume_estimate` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and (`s1`.`volume_date` < `s2`.`volume_date`)))) where isnull(`s2`.`store_id`) order by `s1`.`store_id`;

-- Dumping structure for view mtn_rest.most_recent_store_volume_projection
DROP VIEW IF EXISTS `most_recent_store_volume_projection`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_volume_projection`;
CREATE VIEW `most_recent_store_volume_projection` AS select s1.* from (`store_volume_projection` `s1` left join `store_volume_projection` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and (`s1`.`volume_date` < `s2`.`volume_date`)))) where isnull(`s2`.`store_id`) order by `s1`.`store_id`;

-- Dumping structure for view mtn_rest.most_recent_store_volume_reported
DROP VIEW IF EXISTS `most_recent_store_volume_reported`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_volume_reported`;
CREATE VIEW `most_recent_store_volume_reported` AS select s1.* from (`store_volume_reported` `s1` left join `store_volume_reported` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and (`s1`.`volume_date` < `s2`.`volume_date`)))) where isnull(`s2`.`store_id`) order by `s1`.`store_id`;

-- Dumping structure for view mtn_rest.most_recent_store_volume_third_party
DROP VIEW IF EXISTS `most_recent_store_volume_third_party`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `most_recent_store_volume_third_party`;
CREATE VIEW `most_recent_store_volume_third_party` AS select s1.* from (`store_volume_third_party` `s1` left join `store_volume_third_party` `s2` on(((`s1`.`store_id` = `s2`.`store_id`) and (`s1`.`volume_date` < `s2`.`volume_date`)))) where isnull(`s2`.`store_id`) order by `s1`.`store_id`;
