...
-- Dumping structure for view active_and_future_stores
...
CREATE TABLE `active_and_future_stores` (
	...
	`store_fit` VARCHAR(255) NULL COLLATE 'utf8_general_ci',
	...
	`cbsa_id` INT(11) NULL,
	`sc_condition_score` BINARY(0) NULL,
	`volume_confidence` BINARY(0) NULL,
	`power` FLOAT NULL,
	`power_date` DATE NULL
) ENGINE=MyISAM;
...
CREATE TABLE `active_stores_with_strengths` (
	...
	`volume_total` BIGINT(11) NULL,
	`volume_date` DATE NULL,
	`shopping_center_feature_score` DECIMAL(35,1) NULL,
	`rating_synergy` ENUM('POOR','FAIR','AVERAGE','GOOD','EXCELLENT') NULL COLLATE 'utf8_general_ci',
	`tenant_vacant_count` INT(11) NULL,
	`tenant_occupied_count` INT(11) NULL,
	`parking_space_count` INT(11) NULL,
...
CREATE TABLE `most_recent_shopping_center_survey` (
	...
	`flow_rating` ENUM('POOR','FAIR','AVERAGE','GOOD','EXCELLENT') NULL COLLATE 'utf8_general_ci',
	`tenant_occupied_count` INT(11) NULL,
	`tenant_vacant_count` INT(11) NULL,
...
CREATE TABLE `most_recent_store_survey` (
	...
	`seasonality_notes` TEXT NULL COLLATE 'utf8_general_ci'
) ENGINE=MyISAM;
...
CREATE VIEW `active_and_future_stores` AS select `st`.`store_id` AS `store_id`,`si`.`latitude` AS `latitude`,`si`.`longitude` AS `longitude`,`si`.`address_1` AS `address_1`,`si`.`address_2` AS `address_2`,`si`.`city` AS `city`,`si`.`county` AS `county`,`si`.`state` AS `state`,`si`.`postal_code` AS `postal_code`,`si`.`quad` AS `quad`,`si`.`intersection_street_primary` AS `intersection_street_primary`,`si`.`intersection_street_secondary` AS `intersection_street_secondary`,`st`.`store_type` AS `store_type`,`st`.`store_name` AS `store_name`,`st`.`store_number` AS `store_number`,`stat`.`status` AS `store_status`,(case when (`strength`.`strength` > 1) then 1 when (`strength`.`strength` < 0) then 0 else `strength`.`strength` end) AS `store_strength`,coalesce(`st`.`store_fit`,`b`.`default_store_fit`,'Conventional') AS `store_fit`,(case when (isnull(`st`.`store_fit`) and isnull(`b`.`default_store_fit`)) then 'TRUE' else NULL end) AS `using_default_fit`,`b`.`banner_id` AS `banner_id`,`b`.`banner_name` AS `banner_name`,`c1`.`company_id` AS `company_id`,`c1`.`company_name` AS `company_name`,`c2`.`company_id` AS `parent_company_id`,`c2`.`company_name` AS `parent_company_name`,coalesce(`st`.`area_sales`,`b`.`default_sales_area`) AS `area_sales`,`st`.`area_total` AS `area_total`,`sbv`.`decision` AS `volume_choice`,`sbv`.`volume_total` AS `volume_total`,`sbv`.`volume_date` AS `volume_date`,NULL AS `shopping_center_feature_score`,`mrscc`.`rating_synergy` AS `rating_synergy`,`mrscs`.`tenant_vacant_count` AS `tenant_vacant_count`,`mrscs`.`tenant_occupied_count` AS `tenant_occupied_count`,`mrss`.`parking_space_count` AS `parking_space_count`,`st`.`legacy_location_id` AS `legacy_location_id`,`si`.`cbsa_id` AS `cbsa_id`,NULL AS `sc_condition_score`,NULL AS `volume_confidence`,`mrsm`.`fit_adjusted_power` AS `power`,`mrsm`.`model_date` AS `power_date` from ((((((((((((`site` `si` join `store` `st` on((`si`.`site_id` = `st`.`site_id`))) left join `banner` `b` on((`b`.`banner_id` = `st`.`banner_id`))) left join `company` `c1` on((`c1`.`company_id` = `b`.`company_id`))) left join `company` `c2` on((`c2`.`company_id` = `c1`.`parent_company_id`))) left join `store_strength` `strength` on((`strength`.`store_id` = `st`.`store_id`))) left join `most_recent_store_survey` `mrss` on((`mrss`.`store_id` = `st`.`store_id`))) left join `most_recent_store_casing` `mrsc` on((`mrsc`.`store_id` = `st`.`store_id`))) left join `most_recent_store_model` `mrsm` on((`mrsm`.`store_id` = `st`.`store_id`))) left join `most_recent_store_status` `stat` on((`stat`.`store_id` = `st`.`store_id`))) left join `store_best_volume` `sbv` on((`sbv`.`store_id` = `st`.`store_id`))) left join `most_recent_shopping_center_survey` `mrscs` on((`mrscs`.`shopping_center_id` = `si`.`shopping_center_id`))) left join `most_recent_shopping_center_casing` `mrscc` on((`mrscc`.`shopping_center_id` = `si`.`shopping_center_id`))) where ((`st`.`store_type` <> 'HISTORICAL') and isnull(`si`.`deleted_date`) and isnull(`st`.`deleted_date`));

-- Dumping structure for view active_stores_with_strengths
DROP VIEW IF EXISTS `active_stores_with_strengths`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `active_stores_with_strengths`;
CREATE VIEW `active_stores_with_strengths` AS select `st`.`store_id` AS `store_id`,`si`.`latitude` AS `latitude`,`si`.`longitude` AS `longitude`,`si`.`address_1` AS `address_1`,`si`.`address_2` AS `address_2`,`si`.`city` AS `city`,`si`.`county` AS `county`,`si`.`state` AS `state`,`si`.`postal_code` AS `postal_code`,`si`.`quad` AS `quad`,`si`.`intersection_street_primary` AS `intersection_street_primary`,`si`.`intersection_street_secondary` AS `intersection_street_secondary`,`st`.`store_type` AS `store_type`,`st`.`store_name` AS `store_name`,`st`.`store_number` AS `store_number`,`stat`.`status` AS `store_status`,(case when (`strength`.`strength` > 1) then 1 when (`strength`.`strength` < 0) then 0 else `strength`.`strength` end) AS `store_strength`,coalesce(`st`.`store_fit`,`b`.`default_store_fit`,'Conventional') AS `store_fit`,(case when (isnull(`st`.`store_fit`) and isnull(`b`.`default_store_fit`)) then 'TRUE' else NULL end) AS `using_default_fit`,`b`.`banner_id` AS `banner_id`,`b`.`banner_name` AS `banner_name`,`c1`.`company_id` AS `company_id`,`c1`.`company_name` AS `company_name`,`c2`.`company_id` AS `parent_company_id`,`c2`.`company_name` AS `parent_company_name`,coalesce(`st`.`area_sales`,`b`.`default_sales_area`) AS `area_sales`,`st`.`area_total` AS `area_total`,`sbv`.`decision` AS `volume_choice`,`sbv`.`volume_total` AS `volume_total`,`sbv`.`volume_date` AS `volume_date`,`scs`.`score` AS `shopping_center_feature_score`,`mrscc`.`rating_synergy` AS `rating_synergy`,`mrscs`.`tenant_vacant_count` AS `tenant_vacant_count`,`mrscs`.`tenant_occupied_count` AS `tenant_occupied_count`,`mrss`.`parking_space_count` AS `parking_space_count`,`st`.`legacy_location_id` AS `legacy_location_id` from ((((((((((((`site` `si` join `store` `st` on((`si`.`site_id` = `st`.`site_id`))) left join `banner` `b` on((`b`.`banner_id` = `st`.`banner_id`))) left join `company` `c1` on((`c1`.`company_id` = `b`.`company_id`))) left join `company` `c2` on((`c2`.`company_id` = `c1`.`parent_company_id`))) left join `store_strength` `strength` on((`strength`.`store_id` = `st`.`store_id`))) left join `most_recent_store_survey` `mrss` on((`mrss`.`store_id` = `st`.`store_id`))) left join `most_recent_store_casing` `mrsc` on((`mrsc`.`store_id` = `st`.`store_id`))) left join `store_best_volume` `sbv` on((`sbv`.`store_id` = `st`.`store_id`))) left join `most_recent_store_status` `stat` on((`stat`.`store_id` = `st`.`store_id`))) left join `most_recent_shopping_center_survey` `mrscs` on((`mrscs`.`shopping_center_id` = `si`.`shopping_center_id`))) left join `most_recent_shopping_center_casing` `mrscc` on((`mrscc`.`shopping_center_id` = `si`.`shopping_center_id`))) left join `shopping_center_score` `scs` on((`scs`.`shopping_center_survey_id` = `mrscs`.`shopping_center_survey_id`))) where (`st`.`store_type` = 'ACTIVE');
...
CREATE VIEW `store_strength` AS select `sb`.`store_id` AS `store_id`,round((((`sb`.`volume_total` / coalesce(`st`.`area_sales`,`b`.`default_sales_area`)) - 0.95) / 149.05),3) AS `strength` from ((`store` `st` join `store_best_volume` `sb` on((`st`.`store_id` = `sb`.`store_id`))) left join `banner` `b` on((`b`.`banner_id` = `st`.`banner_id`))) order by `sb`.`store_id`;

-- Dumping structure for view store_volume_actual
DROP VIEW IF EXISTS `store_volume_actual`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `store_volume_actual`;
CREATE VIEW `store_volume_actual` AS select `store_volume`.`store_volume_id` AS `store_volume_id`,`store_volume`.`store_id` AS `store_id`,`store_volume`.`volume_total` AS `volume_total`,`store_volume`.`volume_date` AS `volume_date`,`store_volume`.`volume_type` AS `volume_type`,`store_volume`.`source` AS `source`,`store_volume`.`volume_grocery` AS `volume_grocery`,`store_volume`.`volume_percent_grocery` AS `volume_percent_grocery`,`store_volume`.`volume_meat` AS `volume_meat`,`store_volume`.`volume_percent_meat` AS `volume_percent_meat`,`store_volume`.`volume_non_food` AS `volume_non_food`,`store_volume`.`volume_percent_non_food` AS `volume_percent_non_food`,`store_volume`.`volume_other` AS `volume_other`,`store_volume`.`volume_percent_other` AS `volume_percent_other`,`store_volume`.`volume_produce` AS `volume_produce`,`store_volume`.`volume_percent_produce` AS `volume_percent_produce`,`store_volume`.`volume_plus_minus` AS `volume_plus_minus`,`store_volume`.`volume_note` AS `volume_note`,`store_volume`.`volume_confidence` AS `volume_confidence`,`store_volume`.`created_date` AS `created_date`,`store_volume`.`created_by` AS `created_by`,`store_volume`.`updated_date` AS `updated_date`,`store_volume`.`updated_by` AS `updated_by`,`store_volume`.`deleted_date` AS `deleted_date`,`store_volume`.`deleted_by` AS `deleted_by`,`store_volume`.`version` AS `version`,`store_volume`.`legacy_casing_id` AS `legacy_casing_id` from `store_volume` where ((`store_volume`.`volume_type` = 'ACTUAL') and isnull(`store_volume`.`deleted_date`));

-- Dumping structure for view store_volume_estimate
DROP VIEW IF EXISTS `store_volume_estimate`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `store_volume_estimate`;
CREATE VIEW `store_volume_estimate` AS select `store_volume`.`store_volume_id` AS `store_volume_id`,`store_volume`.`store_id` AS `store_id`,`store_volume`.`volume_total` AS `volume_total`,`store_volume`.`volume_date` AS `volume_date`,`store_volume`.`volume_type` AS `volume_type`,`store_volume`.`source` AS `source`,`store_volume`.`volume_grocery` AS `volume_grocery`,`store_volume`.`volume_percent_grocery` AS `volume_percent_grocery`,`store_volume`.`volume_meat` AS `volume_meat`,`store_volume`.`volume_percent_meat` AS `volume_percent_meat`,`store_volume`.`volume_non_food` AS `volume_non_food`,`store_volume`.`volume_percent_non_food` AS `volume_percent_non_food`,`store_volume`.`volume_other` AS `volume_other`,`store_volume`.`volume_percent_other` AS `volume_percent_other`,`store_volume`.`volume_produce` AS `volume_produce`,`store_volume`.`volume_percent_produce` AS `volume_percent_produce`,`store_volume`.`volume_plus_minus` AS `volume_plus_minus`,`store_volume`.`volume_note` AS `volume_note`,`store_volume`.`volume_confidence` AS `volume_confidence`,`store_volume`.`created_date` AS `created_date`,`store_volume`.`created_by` AS `created_by`,`store_volume`.`updated_date` AS `updated_date`,`store_volume`.`updated_by` AS `updated_by`,`store_volume`.`deleted_date` AS `deleted_date`,`store_volume`.`deleted_by` AS `deleted_by`,`store_volume`.`version` AS `version`,`store_volume`.`legacy_casing_id` AS `legacy_casing_id` from `store_volume` where ((`store_volume`.`volume_type` = 'ESTIMATE') and isnull(`store_volume`.`deleted_date`));

-- Dumping structure for view store_volume_projection
DROP VIEW IF EXISTS `store_volume_projection`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `store_volume_projection`;
CREATE VIEW `store_volume_projection` AS select `store_volume`.`store_volume_id` AS `store_volume_id`,`store_volume`.`store_id` AS `store_id`,`store_volume`.`volume_total` AS `volume_total`,`store_volume`.`volume_date` AS `volume_date`,`store_volume`.`volume_type` AS `volume_type`,`store_volume`.`source` AS `source`,`store_volume`.`volume_grocery` AS `volume_grocery`,`store_volume`.`volume_percent_grocery` AS `volume_percent_grocery`,`store_volume`.`volume_meat` AS `volume_meat`,`store_volume`.`volume_percent_meat` AS `volume_percent_meat`,`store_volume`.`volume_non_food` AS `volume_non_food`,`store_volume`.`volume_percent_non_food` AS `volume_percent_non_food`,`store_volume`.`volume_other` AS `volume_other`,`store_volume`.`volume_percent_other` AS `volume_percent_other`,`store_volume`.`volume_produce` AS `volume_produce`,`store_volume`.`volume_percent_produce` AS `volume_percent_produce`,`store_volume`.`volume_plus_minus` AS `volume_plus_minus`,`store_volume`.`volume_note` AS `volume_note`,`store_volume`.`volume_confidence` AS `volume_confidence`,`store_volume`.`created_date` AS `created_date`,`store_volume`.`created_by` AS `created_by`,`store_volume`.`updated_date` AS `updated_date`,`store_volume`.`updated_by` AS `updated_by`,`store_volume`.`deleted_date` AS `deleted_date`,`store_volume`.`deleted_by` AS `deleted_by`,`store_volume`.`version` AS `version`,`store_volume`.`legacy_casing_id` AS `legacy_casing_id` from `store_volume` where ((`store_volume`.`volume_type` = 'PROJECTION') and isnull(`store_volume`.`deleted_date`));
...
CREATE VIEW `store_volume_third_party` AS select `store_volume`.`store_volume_id` AS `store_volume_id`,`store_volume`.`store_id` AS `store_id`,`store_volume`.`volume_total` AS `volume_total`,`store_volume`.`volume_date` AS `volume_date`,`store_volume`.`volume_type` AS `volume_type`,`store_volume`.`source` AS `source`,`store_volume`.`volume_grocery` AS `volume_grocery`,`store_volume`.`volume_percent_grocery` AS `volume_percent_grocery`,`store_volume`.`volume_meat` AS `volume_meat`,`store_volume`.`volume_percent_meat` AS `volume_percent_meat`,`store_volume`.`volume_non_food` AS `volume_non_food`,`store_volume`.`volume_percent_non_food` AS `volume_percent_non_food`,`store_volume`.`volume_other` AS `volume_other`,`store_volume`.`volume_percent_other` AS `volume_percent_other`,`store_volume`.`volume_produce` AS `volume_produce`,`store_volume`.`volume_percent_produce` AS `volume_percent_produce`,`store_volume`.`volume_plus_minus` AS `volume_plus_minus`,`store_volume`.`volume_note` AS `volume_note`,`store_volume`.`volume_confidence` AS `volume_confidence`,`store_volume`.`created_date` AS `created_date`,`store_volume`.`created_by` AS `created_by`,`store_volume`.`updated_date` AS `updated_date`,`store_volume`.`updated_by` AS `updated_by`,`store_volume`.`deleted_date` AS `deleted_date`,`store_volume`.`deleted_by` AS `deleted_by`,`store_volume`.`version` AS `version`,`store_volume`.`legacy_casing_id` AS `legacy_casing_id` from `store_volume` where ((`store_volume`.`volume_type` = 'THIRD_PARTY') and isnull(`store_volume`.`deleted_date`));
